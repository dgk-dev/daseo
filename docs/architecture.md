...
>
> **In-app browser window opens.** Ordinary link opens, including Shift-clicked links, become Paseo workspace tabs. Script-created opens with popup features or a named target and POST-backed opens keep Chromium's original popup `WebContents`, but Electron adopts it into the opener browser's target graph instead of showing an OS child window. This preserves `window.opener`, `postMessage`, named-window reuse, request bodies, recursive popups, and `window.close()` for OAuth and similar protocols. A background popup stays in a never-shown, non-focusable native parking window with its real viewport; selecting it reparents the same `WebContentsView` into the opener's browser pane. Unsupported URL schemes are denied before either path.
>
